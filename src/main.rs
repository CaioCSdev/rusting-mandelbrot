...
fn main() {
    ...
    if args.len() != 5 {
        writeln!(std::io::stderr(), "Usage: mandelbrot FILE PIXELS UPPERLEFT LOWERRIGTH").unwrap();
    ...
    let upper_left = parse_complex(&args[3]).expect("Error parsing image dimensions");
    let lower_right = parse_complex(&args[4]).expect("Error parsing image dimensions");
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<i32>("10,20asdf", ','), None);
